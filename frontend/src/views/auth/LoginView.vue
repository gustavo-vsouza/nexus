<template>
  <div class="grid grid-cols-1 md:grid-cols-2 h-screen">
    <!-- Coluna Esquerda -->
    <div class="bg-[#2d5594] text-white flex-col justify-center items-center p-10 text-center hidden md:flex">
    ...
    <div class="flex flex-col items-center justify-center px-6 py-10 bg-primary-dark">
      ...
      <div class="w-full max-w-sm space-y-4">
        <InputComponent v-model="email" tipo="email" label="E-mail" placeholder="Digite seu e-mail" icon="mail" required/>

        <div class="w-full">
          <InputComponent v-model="senha" tipo="password" label="Senha" icon="password" required />
          <p class="text-xs text-blue-700 cursor-pointer hover:underline mt-1 text-left">
            Esqueci minha senha
          </p>
        </div>
      </div>
      ...
      <div class="w-full max-w-sm mt-6 space-y-3">
        <button
          class="cursor-pointer w-full py-2 bg-blue-700 text-white font-semibold rounded-lg shadow-md hover:bg-blue-800 transition"
          @click="login()">
          Entrar
        </button>
      ...
      <p class="mt-6 text-sm text-gray-700">
        ...
        <RouterLink to="/cadastro" class="text-blue-700 font-medium hover:underline">Cadastrar-se</RouterLink>
      </p>
    </div>
  </div>
</template>
